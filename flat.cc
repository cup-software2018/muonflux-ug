...
double AngleCorrection(double theta);
double ModifiedGaisser(double E_mu, double theta_rad);

int main(int argc, char ** argv)
{
  // initialize music
  const char * data1 = "/home/cupsoft/Works/Muon/muonflux-ug/music-eloss-sr.dat";
  const char * data2 = "/home/cupsoft/Works/Muon/muonflux-ug/music-double-diff-rock.dat";
  const char * data3 = "/home/cupsoft/Works/Muon/muonflux-ug/music-cross-sections-sr.dat";
...
double ModifiedGaisser(double E_mu, double theta_rad)
{
  double theta = cos(theta_rad);
  double theta_star = AngleCorrection(theta);
  ...
  double E_corr = E_mu * (1.0 + E_mu_decay / (E_mu * theta_star));
  double spectrum = pow(E_corr, -gamma);

  double term_pi = 1.0 / (1.0 + 1.1 * E_mu * theta_star / eps_pi);
  double term_K = r_K_pi / (1.0 + 1.1 * E_mu * theta_star / eps_K);
